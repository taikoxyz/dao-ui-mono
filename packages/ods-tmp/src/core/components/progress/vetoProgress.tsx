...
export const VetoProgress: React.FC<IProgressProps> = (props) => {
    const {
        value,
        size = 'md',
        responsiveSize,
    ...
    const containerClassNames = classNames('relative flex w-full rounded-xl bg-neutral-100', sizeClassNames, className);
        ...
        <RadixProgress.Root value={processedValue} className={containerClassNames} {...otherProps}>
            <RadixProgress.Indicator
                className={classNames(
                    ...
                    variantToClassNames['success']
                )}
                style={{ width: `${100 - processedValue}%` }}
            />
            <RadixProgress.Indicator
                className={classNames(
                    `h-full rounded-r-xl transition-[border-radius,width] duration-500 ease-in-out`,
