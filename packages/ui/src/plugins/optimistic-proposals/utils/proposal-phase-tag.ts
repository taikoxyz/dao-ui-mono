 ...
 */
export function getPhaseTag({ status, isEmergency, isL2GracePeriod, isTimelockPeriod }: PhaseTagInput): PhaseTag {
  // Status is briefly undefined while useProposalStatus computes.
  if (!status) return { label: "Pending", variant: "neutral" };

  // Terminal states.
  if (status === ProposalStatus.VETOED) return { label: "Defeated", variant: "critical" };
