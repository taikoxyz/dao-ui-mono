...
export const VetoStatus: FC<VetoStatusProps> = ({
  vetoCount,
  totalSupply,
  threshold,
  status,
  ...
  return (
    <div className="flex flex-col gap-4 p-4 bg-neutral-0 rounded-xl border border-neutral-100">
      {/* Header */}
      ...
      {!thresholdReached && status === 'active' && (
        <div className="text-sm text-neutral-600 bg-primary-50 p-3 rounded-lg">
          <span className="font-medium">{(thresholdPercentage - vetoPercentage).toFixed(2)}%</span> more vetoes needed to defeat this proposal
        </div>
      )}
    </div>
  );
